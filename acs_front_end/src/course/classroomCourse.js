...
export default function ClassroomCourse() {
  ...
  const [classroom_name, SetClassroomName] = useState("null");

  const params = useParams();
  var id = params.classroom_id;
  ...
  useEffect(() => {
    axios
      .get("http://127.0.0.1:5000/api/classrooms/courses/" + id)
      .then((response) => {
    ...
    axios // get classroom name
      .get("http://127.0.0.1:5000/api/classrooms/name/" + id)
      .then((response) => {
  ...
  return (
    <div className="course" id="print-content">
      ...
      <div className="schedule" ref={contentToPrint}>
        <div className="title">{classroom_name}课表</div>
        <div className="body">
