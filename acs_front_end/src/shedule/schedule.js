...
export default function Schedule() {
  ...
  useEffect(() => {
    if (changeid != -1) {
      ...
      // ];
      axios
        .post("http://127.0.0.1:5000/api/teacher/change/class", data)
        .then((response) => {
  ...
  const change_class = (class_id) => {
    ...
    axios
      .post("http://127.0.0.1:5000/api/change/schedule/classroom", data)
      .then((response) => {
        console.log(response.data);
  ...
  useEffect(() => {
    if (selecttime1 != 0 && selecttime2 != 0) {
      ...
      console.log(data);
      axios
        .post("http://127.0.0.1:5000//api/teacher/change/time", data)
        .then((response) => {
          console.log(response.data);
          const data_class2 = [];
          const campusid2str = [
  ...
  const change_time = (class_id) => {
    ...
    console.log(data);
    axios
      .post("http://127.0.0.1:5000/api/change/schedule/time", data)
      .then((response) => {
        console.log(response.data);
  ...
  const items_time = [
    ...
    },
    {
      ...
      label: "可用教室列表",
      children: (
        <Table columns={columns_class_time} dataSource={class_data_time} />
      ),
    },
  ];
  const reschedule = () => {
    axios
      .post("http://127.0.0.1:5000/api/reschedule")
      .then((response) => {
        console.log(response.data);
        Setschedule_data(response.data.schedules);
        message.success("自动排课成功!");
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  return (
    <div className="schedule_list">
      ...
      />
      <Button type="primary" onClick={reschedule}>
        重新调课
      </Button>
